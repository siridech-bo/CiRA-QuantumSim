...
> Remaining `[TBD]`s are optional refinements (E4 recursive-GBC depth, full non-RWA), noted in place.
...
- **E5 — Chain scaling: is the trade-off robust to spectator modes?** *(done).* We built a
  ...
  **Result.** The incoherent sensitivity is **target-mode dominated**. For a 2-ion axial gate
  (COM target, stretch spectator, $\bar n=0.5$ on each), the per-mode incoherent slope
  $\mathrm{d}(1{-}F)/\mathrm{d}\kappa$ is $8.09$ for the driven COM mode but only $0.87$ for the
  stretch spectator — a ratio of $0.11$. Physically, heating decoheres the qubit–motion
  entanglement, which is $O(1)$ for the driven mode but small for a detuned spectator. Hence a
  spectator mode raises the effective incoherent floor $I_{\rm incoh}$ by only $\sim\!10\%$, and
  the trade-off crossover ($\Delta\omega^\times\!\propto\!\sqrt{I_{\rm incoh}}$, E3) shifts by just
  $\sqrt{1.1}\approx1.05$ ($\sim\!5\%$). **The single-mode trade-off (E1–E3) therefore carries over
  to a real $N$-ion chain essentially unchanged** — spectator-mode load does not move the
  robust-vs-fast verdict. (This is the incoherent counterpart of the coherent spectator residual,
  which U1 waveform-shaping closes; here the *irreducible* incoherent part is shown to be benign.)
...
grows as $4\prod_m N_{\rm Fock}^{(m)}$, so the open-system study is run at $M\!\le\!2$
modes (the $O(\dim^2 M)$ sparse-Hermitian evaluation keeps this tractable; larger $M$ is
mechanical but costly) — sufficient to establish target-mode dominance (E5); single-qubit
