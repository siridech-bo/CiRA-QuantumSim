...
The robust waveform above suppresses the \emph{symmetric} (motional-frequency) error axis by pulse
shaping; a structurally different route to the \emph{same} axis is the smooth gate of Hughes
\emph{et~al.}~\cite{Hughes25}: hold the drive envelope $\Omega$ essentially constant and instead ramp
the detuning $\delta(t)$ \emph{slowly}, so the spin-dependent displacement is eliminated
\emph{adiabatically} rather than by a designed closure. In the forced ($\pm$) spin sector,
$\hat H=\delta(t)a^\dagger a+\Omega(t)(a+a^\dagger)$ and the displacement $\alpha(t)=\langle a\rangle$
obeys the driven-oscillator equation
\begin{equation}
...
nothing to the center-line $\sigma_z$), so it is orthogonal to the asymmetric-axis compensation
(GBC, Sec.~\ref{sec:gbc}, next) --- their leading infidelities add. But the adiabatic sweep makes
the gate \emph{long} ($\tau_{\rm smooth}\!\approx\!14\tau$ at the parameters of E7), and since the
...
\subsection{Generator-based compensation}\label{sec:gbc}
The two symmetric-axis methods above leave the \emph{asymmetric} (center-line) axis untouched; it
needs a different tool. Not every error can be refocused: a trailing single-qubit rotation removes any error commuting
...
Everything above --- mode closure, robust shaping, the smooth-gate adiabatic elimination, and GBC
--- is a \emph{coherent} design: it optimizes a unitary while treating the environment as absent. This subsection restores the
...
\section{Toolchain}
\begin{figure}[t]\includegraphics[width=\columnwidth]{Fig1_toolchain_v2}
...
\emph{B3 (RWA + Lamb--Dicke validity).} Integrating the exact spin-dependent-force Hamiltonian
$H=\omega_z a^\dagger a+\Omega\sum_j\sigma_x^j\sin[\eta(a{+}a^\dagger)]\cos[(\omega_z{-}\delta)t]$
...
$\lesssim\!10^{-4}$, so the RWA+LD model underlying the numerical experiments (Sec.~VI) is accurate;
the exact model marks the boundary (fast/strong drive, or $\eta\gtrsim0.3$) beyond which it should be
replaced.

Together, B1--B3 fix a common ground truth: any fidelity change reported
...
\begin{figure}[t]\includegraphics[width=\columnwidth]{Fig6_smooth_gbc}
