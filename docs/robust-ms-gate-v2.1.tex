...
we reproduce its motional-robustness signatures (Sec.~\ref{sec:smooth}, \ref{sec:e7}) and use it as
...
boundary with an add-GBC threshold $\Delta\omega^\times\!\approx\!3.5\times10^{-3}$ (Sec.~\ref{sec:e7}),
...
($\tau_{\rm smooth}\!\approx\!14\tau$ at the parameters of Sec.~\ref{sec:e7}), and since the
center-line error enters as $\varepsilon=\Delta\omega\tau$, the smooth gate is \emph{more} exposed to
the asymmetric axis than a short gate --- exactly the case where appending GBC pays (Sec.~\ref{sec:e7}).
...
function, and drives the four-scheme combining map of Sec.~\ref{sec:e7}). \emph{(E) Cross-validation}: analytic and numerical Bell fidelities agree to
...
\paragraph{E7 --- smooth gate $\times$ GBC.}\label{sec:e7} E6 compared two robust waveforms on the
...
+\underbrace{c(\Delta\omega\tau)}_{\text{asymmetric (GBC)}}
...
smooth-gate displacement & $\dot\alpha=-i(\delta(t)\alpha+\Omega(t))$ & \textsf{ion-smooth} (\textsf{integrateAlpha}) \\
adiabatic ramp & $\delta(s)=(b{+}c\,g(s))^{-1/j}$ & \textsf{ion-smooth} (\textsf{smoothProtocol}) \\
motional-noise filter & $F(\omega)$ & \textsf{ion-smooth} (\textsf{filterFunction}) \\
smooth$\times$GBC map & $1{-}F(\Delta\delta,\Delta\omega)$ & \textsf{ion-smooth} (\textsf{schemes}, \textsf{winner}) \\
